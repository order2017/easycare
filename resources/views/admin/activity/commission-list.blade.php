...
@section('content')
    <div class="lion-content-header am-cf">
        <h3 class="am-fl">导购红包活动管理</h3>
        <a href="{{route('admin.activity.commission.page')}}" class="am-btn am-btn-secondary am-fr am-btn-sm">新增活动
        </a>
        <a href="" class="am-btn am-btn-default am-fr am-btn-sm">刷新
        </a>
        <button type="button" data-am-modal="{target: '#modaldg-search'}"
    ...
    <table class="am-table am-table-bordered am-animation-slide-bottom am-table-striped am-table-hover am-table-compact am-table-centered">
        ...
        @foreach($list as $line)
            <tr>
                ...
                <td>{{$line['end_time']}}</td>
                <td>{{$line['rules']['total']}}</td>
                <td>{{$line['has_join_num']}}</td>
                <td>{{$line['created_at']}}</td>
                <td>{{$line['status_text']}}</td>
                <td>
                    <a href="{{route('admin.activity.commission.page',['id'=>$line['id']])}}"
                       class="am-btn am-btn-success am-btn-xs">修改</a>
                    <form action="{{route('admin.activity.commission.up-or-down',['id'=>$line['id']])}}" method="post"
                          class="am-inline">
                        {{method_field('PUT')}}
                        @if($line['is_down'])
                    ...
                    <form action="{{route('admin.activity.commission.delete',['id'=>$line['id']])}}" method="post"
...
@endsection
<div class="am-modal am-modal-no-btn" tabindex="-1" id="modaldg-search">
