...
@section('content')
    <div class="lion-content-header am-cf">
        <h3 class="am-fl">新增/修改会员积分活动</h3>
    </div>
    <form method="post" class="am-form" action="">
        <input type="hidden" name="type" value="{{\App\ProductActivity::TYPE_MEMBER_INTEGRAL}}">
        <div class="am-g">
            <div class="am-u-sm-6">
                <div class="am-g">
                    <div class="am-u-sm-4">
                    ...
                    <div class="am-u-sm-4">
                        <div class="am-form-group">
                            <label for="doc-ipt-email-1">奖励倍数</label>
                            <div class="am-input-group">
                                <input type="text" name="rules[multiple]" id="doc-ipt-email-1"
                                       value="{{$model['rules']['multiple']}}"
                                       placeholder="请输入奖励倍数">
                                ...
                                @if(!empty($model['products']))
                                    @foreach($model->getProducts($model['type'],$model['begin_time'],$model['end_time'],$model['id']) as $item)
                                        <option value="{{$item['id']}}"
                                                @if(in_array($item['id'],isset($model['products']) ? $model['products'] : [])) selected @endif>{{$item['model']}}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                ...
                <div class="am-u-sm-12 am-text-center">
                    ...
                    <a class="am-btn am-btn-default inline-block"
                       href="{{route('admin.activity.integral.list')}}">返回</a>
                </div>
            </div>
        </div>
    </form>
